...
std::vector<double> compute_gradients(const std::vector<lib::KddSample*>& samples,const std::vector<Key>& keys, const std::vector<double>& vals, double alpha) {
  ...
  for (auto* sample : samples) {
     ...
     auto& x = sample->x_;
            ...
            for (auto field : x) {
                while (keys[j] < field.first)
                    j += 1;
                deltas[j] += alpha * field.second * (y - predict);
            }
...
double error(const std::vector<lib::KddSample*>& samples, const std::vector<Key>& keys,
...
void LRTest() {
  ...
  task.SetLambda([kTable, &data_store, node_id](const Info& info) {
    ...
    auto error_after = error(datasample_3, keys_3, vals_3);

    LOG(INFO) << "After learning, Node_id: " << node_id << " Correctrate: " << error_after;
    table.Clock();
  });
