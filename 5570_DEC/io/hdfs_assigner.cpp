...
void HDFSBlockAssigner::handle_exit() {
  ...
  LOG(INFO) << "master => worker finished @" << worker_name << "-" << std::to_string(worker_id) << " Finished workers: " << finished_workers_.size();
  
  if ((finished_workers_.size() == num_workers_alive_)) {
    LOG(INFO) << worker_name << "halted";
    halt();
  }
}
...
void HDFSBlockAssigner::handle_block_request(const std::string& cur_client) {
  ...
  //LOG(INFO) << host << " " << num_threads << " " << id << " " << load_type;
